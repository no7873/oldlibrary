...
class Rcart(object):
    ...
    def __len__(self): # 장바구니에 들어있는 제품의 quantity 항목들을 전부 더한 결과 제공
        return sum(item['quantity'] for item in self.rcart.values())
    ...
    def __iter__(self): # for 문 등 사용할 때 어떤 요소들을 건네줄 것인지 지정
        ...
        for item in self.rcart.values(): # 장바구니에 들어있는 제품들을 하나씩 꺼내는 것
    ...
    def get_product_total(self):
        return sum(Decimal(item['price']) * item['quantity'] for item in self.rcart.values())
